...
public class UpdateTeacherFormController implements Initializable {
    ...
    private void searchResult() throws SQLException, ClassNotFoundException {
        ResultSet result = crudUtil.execute("SELECT * FROM teacher WHERE teacherId=? OR teacherName=?",checkID.getText(),checkName.getText());
        if (result.next()) {
    ...
    private void updateDetail() {
        teacher t1=new teacher(newID.getText(),newName.getText(),newAddress.getText(),Double.parseDouble(newSalary.getText()),Integer.parseInt(newContact.getText()));

        try{
            if(crudUtil.execute("UPDATE teacher SET teacherName=?,teacherAddress=?,teacherSalary=?,teacherContact=?  WHERE teacherId=?",t1.getTeacherName(),t1.getTeacherAddress(),t1.getTeacherSalary(),t1.getTeacherContact(),t1.getTeacherId())){
    ...
    public void keyReleasedAction(KeyEvent keyEvent) {
        ...
        Pattern comStudentId = Pattern.compile("^(t)[0-9]{4}$");
