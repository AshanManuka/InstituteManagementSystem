...
public class UpdateUserFormController implements Initializable {
    ...
    private void searchResult() throws SQLException, ClassNotFoundException {
        ResultSet result = crudUtil.execute("SELECT * FROM user WHERE userId=? OR userName=?",userID.getText(),checkName.getText());
        if (result.next()) {
    ...
    private void updateDetail() {
        user u1=new user(newID.getText(),newName.getText(),newAddress.getText(),Double.parseDouble(newSalary.getText()),Integer.parseInt(newContact.getText()));

        try{
            if(crudUtil.execute("UPDATE user SET userName=?,userAddress=?,userSalary=?,userContact=?  WHERE userId=?",u1.getUserName(),u1.getUserAddress(),u1.getUserSalary(),u1.getUserContact(),u1.getUserId())){
    ...
    public void keyReleasedAction(KeyEvent keyEvent) {
        ...
        Pattern comStudentId = Pattern.compile("^(u)[0-9]{4}$");
